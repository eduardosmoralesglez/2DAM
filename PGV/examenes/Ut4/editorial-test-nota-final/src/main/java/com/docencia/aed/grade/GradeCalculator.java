...
public class GradeCalculator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean secured = isSecuredProject(projectRoot);
        ...
        // Reglas de cap:
        // - Si NO está securizado, la nota máxima es 3.00
        // - Si está securizado, la nota máxima es 10.00 (comportamiento original)
        double max = secured ? 10.0 : 3.0;

        if (total < 0) total = 0;
        if (total > max) total = max;

        sb.append("MODO: ").append(secured ? "SECURED" : "UNSECURED").append("\n");
        sb.append("NOTA FINAL: ").append(DF.format(total)).append(" / ").append(DF.format(max)).append("\n");

        write(out, sb.toString());
        System.out.println(sb);
    }

    private static boolean isSecuredProject(Path projectRoot) {
        try {
            Path marker = projectRoot.resolve("target").resolve("security-mode.txt");
            if (!Files.exists(marker)) return true; // por defecto, comportamiento original
            String v = Files.readString(marker, StandardCharsets.UTF_8).trim();
            return v.equalsIgnoreCase("SECURED");
        } catch (Exception e) {
            return true;
        }
    }
